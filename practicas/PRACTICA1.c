...
Nodoint * apilar2(Nodoint *cima2,int d)
{
    ...
    char valorf;
      while(tap>0){
       if((exp[p]>=48) && (exp[p]<=57)){
            valorf=exp[p];
        pilaR=apilar2(pilaR,valorf-'0');
       // printf("%d\n",verTop(pilaR));

       }else{
           //printf("1");
            int val1 =verTop2(pilaR) ;
             pilaR=desapilar2(pilaR);
           //printf("2");
            int val2 = verTop2(pilaR);
            pilaR=desapilar2(pilaR);
            switch (exp[p])
            {
            case '+': pilaR=apilar2(pilaR, val2 + val1); break;
            case '-': pilaR=apilar2(pilaR, val2 - val1); break;
            case '*': pilaR=apilar2(pilaR, val2 * val1); break;
            case '/': pilaR=apilar2(pilaR, val2/val1); break;
            case '%': pilaR=apilar2(pilaR, val2%val1); break;
            case '^': pilaR=apilar2(pilaR, pow(val2,val1)); break;
            }
...
printf("%d\n",verTop2(pilaR));
          ...
          fprintf (Archivo3, "Valores sustituidos: %s Evaluacion: %d\n\n\n",cadA, res);
